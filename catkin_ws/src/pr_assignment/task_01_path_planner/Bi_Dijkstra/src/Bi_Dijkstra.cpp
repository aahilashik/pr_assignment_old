...
void Bi_Dijkstra::uploadMap(int (&map2D)[width][height]) {
    mapWidth  = width;
    mapHeight = height;
...
void Bi_Dijkstra::setStartNode(int startX, int startY) {
...
void Bi_Dijkstra::setGoalNode(int goalX, int goalY) {
    // If map is empty/null 
    if (nodes == nullptr) {
        cout << "WARN : Unknown Environment! Kindly update the map.." << endl;
        return;
...
void Bi_Dijkstra::setVisualization(bool enable) {
    visualize = enable;
}

bool Bi_Dijkstra::startPlan() {
...
void Bi_Dijkstra::getPlannedPath(vector<Node*>& path) {
    path.assign(plannedPath.begin(), plannedPath.end());
    return ;    
}

bool Bi_Dijkstra::setPlannedPath(Node *nodes, Node *nodeStart, Node *nodeGoal) {
    ...
    for (int x = 0; x < mapWidth; x++)
        for (int y = 0; y < mapHeight; y++) {
            ...
            nodes[x * mapWidth + y].costMap["StartStepCost"] = INFINITY;
            nodes[x * mapWidth + y].costMap["GoalStepCost"] = INFINITY;
    ...
    while (!openNodesList[0].empty() && !openNodesList[1].empty()) {
        ...
        for (int dir=0; dir<2; dir++) {
            ...
            for (auto nodeNeighbour : nodeCurrent->nodeNeighbours) {
                ...
                float minCost = nodeCurrent->costMap[(dir==0?"StartStepCost":"GoalStepCost")] + distance(nodeCurrent, nodeNeighbour);

                if (minCost < nodeNeighbour->costMap[(dir==0?"StartStepCost":"GoalStepCost")]) {
                    nodeNeighbour->parents[(dir==0?"StartParent":"GoalParent")] = nodeCurrent;
                    nodeNeighbour->costMap[(dir==0?"StartStepCost":"GoalStepCost")] = minCost;
                }
            }	
        }
...
void Bi_Dijkstra::printMap(vector<Node*> path) {
    // Print the Map
    for (int y=0; y<mapHeight; y++)
        cout << "--";
    cout << "---\n";
