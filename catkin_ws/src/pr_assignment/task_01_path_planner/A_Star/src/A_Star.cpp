...
void A_Star::uploadMap(int (&map2D)[width][height]) {
    mapWidth  = width;
    mapHeight = height;
...
void A_Star::setStartNode(int startX, int startY) {
...
void A_Star::setGoalNode(int goalX, int goalY) {
    // If map is empty/null 
    if (nodes == nullptr) {
        cout << "WARN : Unknown Environment! Kindly update the map.." << endl;
        return;
...
void A_Star::setVisualization(bool enable) {
    visualize = enable;
}

bool A_Star::startPlan() {
...
void A_Star::getPlannedPath(vector<Node*>& path) {
    path.assign(plannedPath.begin(), plannedPath.end());
    return ;    
}

bool A_Star::setPlannedPath(Node *nodes, Node *nodeStart, Node *nodeGoal) {
    ...
    for (int x = 0; x < mapWidth; x++)
        for (int y = 0; y < mapHeight; y++) {
            ...
            nodes[x * mapWidth + y].costMap["PathCost"] = INFINITY;
            nodes[x * mapWidth + y].costMap["TotalCost"] = INFINITY;
    ...
    nodeCurrent->costMap["PathCost"] = 0.0f;
    nodeCurrent->costMap["TotalCost"] = heuristic(nodeStart, nodeGoal);
    ...
    while (!openNodesList.empty() && nodeCurrent != nodeGoal) {
        ...
        for (auto nodeNeighbour : nodeCurrent->nodeNeighbours) {
            ...
            float minCost = nodeCurrent->costMap["PathCost"] + distance(nodeCurrent, nodeNeighbour);

            if (minCost < nodeNeighbour->costMap["PathCost"]) {
                nodeNeighbour->parents["Parent"] = nodeCurrent;
                nodeNeighbour->costMap["PathCost"] = minCost;
...
void A_Star::printMap(vector<Node*> path) {
    // Print the Map
    for (int y=0; y<mapHeight; y++)
        cout << "--";
    cout << "---\n";
